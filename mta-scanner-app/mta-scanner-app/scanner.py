...
Compativel com Python 3.7+ (sem dependencias externas).
"""
...
APP_TITLE   = "MTA Store Scanner v2.0"
...
def parse_lua_file(filepath):
    ...
    cmd_pattern = re.compile(r'addCommandHandler\s*\(\s*["\']([^"\']+)["\']', re.IGNORECASE)
    for match in cmd_pattern.finditer(content):
        cmd = match.group(1).strip()
        if cmd and cmd not in result["commands"]:
            result["commands"].append(cmd)

    export_pattern = re.compile(r'^function\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*\(', re.MULTILINE)
    ...
    tier_pattern = re.compile(
        r'(?:name|nome|label|tier|nivel|plano)\s*=\s*["\']([^"\']{2,30})["\']', re.IGNORECASE
    )
    ...
    setdata_pattern = re.compile(
        r'setElementData\s*\([^,]+,\s*["\']([^"\']+)["\']', re.IGNORECASE
    )
...
def parse_meta_xml(filepath):
    info = {"description": "", "author": "", "version": "?", "type": "misc", "scripts": []}
    try:
...
def classify_resource(name, description, lua_data):
    ...
    if is_vip:
        ...
        tier_kws = {
            ...
            "platinum": "VIP Platinum", "vip1": "VIP Nivel 1",
            "vip2": "VIP Nivel 2", "vip3": "VIP Nivel 3",
        ...
        if real_tiers:
            for t in real_tiers:
                sellable.append({
                    ...
                    "suggestedDesc": f"{t['name']} — beneficios exclusivos no servidor.",
                    "category": "vip", "mtaCommand": "giveVip",
                    "mtaParams": {"tier": t["tier_id"], "days": 30, "resource": name},
                    "autoDetected": True, "resourceName": name,
                })
        else:
            sellable.append({
                ...
                "autoDetected": False, "resourceName": name,
            })

    elif any(k in all_text for k in VEHICLE_KEYWORDS):
        category = "vehicle"
        sellable.append({
            "suggestedName": f"Veiculo — {name}",
            "suggestedDesc": "Veiculo entregue no seu spawn.",
...
def scan_resources_folder(folder_path, progress_cb=None):
    ...
    for idx, res_dir in enumerate(sorted(items)):
        ...
        category, sellable, commands = classify_resource(name, meta["description"], lua_data)

        entry = {
            "name": name, "description": meta["description"],
            "author": meta["author"], "version": meta["version"],
            "type": meta["type"], "category": category,
...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        cfg = ttk.LabelFrame(self, text=" Configuracao ", padding=12)
        ...
        ttk.Label(row2, text="Ex: https://mtastore.site", foreground="#64748b").pack(side="left")
    ...
    def _start_scan(self):
        ...
        if not folder or not os.path.isdir(folder):
            messagebox.showerror("Erro", "Selecione uma pasta resources valida.")
            return
        self._save_config()
    ...
    def _scan_done(self):
        ...
        for res in self.resources:
            ...
            for item in res["sellable"]:
                cat  = item["category"]
                auto = "OK" if item["autoDetected"] else ""
                tag  = cat if cat in ("vip", "vehicle", "coins", "item") else "other"
                self.tree.insert("", "end", values=(
                    res["name"], cat.upper(), item["suggestedName"],
                    item["mtaCommand"], auto,
                ), tags=(tag,))
                count += 1

        total_resources = len(self.resources)
        self._log(f"Scan concluido: {total_resources} resources, {count} item(ns) detectado(s).")
        self.lbl_status.configure(text=f"Scan concluido — {count} item(ns) detectado(s) em {total_resources} resources.")
    ...
    def _send_to_site(self):
        ...
        if not validate_url(url):
            messagebox.showerror("Erro", "URL invalida. Comece com https:// (ex: https://mtastore.site)")
            return
        if not token:
    ...
    def _send_thread(self, url, token):
        ...
        try:
            ...
            msg = f"Enviado! {total} item(ns), {created} produto(s) criado(s), {skipped} ja existiam."
            self._log(msg)
            self.after(0, lambda: messagebox.showinfo("Sucesso!", msg))
        except urllib.error.HTTPError as e:
            body = e.read().decode()
            if e.code == 401:
                msg = "Token invalido. Verifique o token nas configuracoes do site."
                self._log(f"ERRO 401: {msg}")
                self.after(0, lambda: messagebox.showerror("Erro", msg))
            elif e.code == 404:
                msg = "Endpoint nao encontrado. Verifique a URL do site."
                self._log(f"ERRO 404: {msg}")
                self.after(0, lambda: messagebox.showerror("Erro", msg))
            else:
                self._log(f"ERRO HTTP {e.code}: {body}")
                self.after(0, lambda: messagebox.showerror("Erro HTTP", f"Codigo {e.code}: {body[:200]}"))
        except Exception as e:
            err_str = str(e)
            if "timed out" in err_str.lower() or "timeout" in err_str.lower():
                msg = "Tempo esgotado. O site pode estar offline."
                self._log(f"ERRO: {msg}")
                self.after(0, lambda: messagebox.showerror("Erro", msg))
            elif "urlopen" in err_str.lower() or "connection" in err_str.lower():
                msg = "Nao foi possivel conectar ao site. Verifique a URL."
                self._log(f"ERRO: {msg}")
                self.after(0, lambda: messagebox.showerror("Erro", msg))
            else:
                self._log(f"ERRO: {e}")
    ...
    def _export_json(self):
        ...
        try:
            with open(path, "w", encoding="utf-8") as f:
                json.dump({"resources": self.resources, "detected": self.detected},
