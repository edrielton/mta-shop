...
if body == "" then respond(400, {success=false, error="Empty body"}); return end

local payload = fromJSON(body)
if type(payload) ~= "table" then respond(400, {success=false, error="Invalid JSON"}); return end
...
if not command or command == "" then respond(400, {success=false, error="Missing command"}); return end
if serial == "" and account == "" then respond(400, {success=false, error="Missing serial or account"}); return end
if transactionId == "" then respond(400, {success=false, error="Missing transactionId"}); return end
