...
local SYNC_INTERVAL = 30 -- segundos

local function log(msg)
    outputServerLog("[MTA-Store Sync] " .. tostring(msg))
end
        ...
        function(response, errno)
            if errno ~= 0 then
                if MTA_STORE_DEBUG then
                    outputDebugString("[MTA-Store] fetchRemote erro "..errno.." em "..endpoint, 2)
                end
                return
            end
...
local function fetchAndSync(player)
    ...
    exports["oxmysql"]:single(
        ...
        function(row)
            ...
            if not row then
                if MTA_STORE_DEBUG then
                    outputDebugString("[MTA-Store] Jogador não encontrado no banco: "..serial, 3)
                end
                return
            end
...
addEventHandler("onPlayerQuit", root, function()
    ...
    postToSite("/api/player/sync", {serial=serial, online=false, updatedAt=os.time()}, nil)
end)
...
local function cmdLoja(player)
    ...
    if not acc or isGuestAccount(acc) then
        chat(player, "Faça login numa conta MTA primeiro (/login).")
        return
    end
...
log("Sync iniciado. Comandos: /loja | /store | /shop")
