...
var StripeSync = class {
  constructor(config) {
    this.config = config;
    this.stripe = new Stripe2(config.stripeSecretKey, {
  ...
  async fetchAndUpsert(fetch2, upsert, accountId, resourceName) {
  ...
  async fetchMissingEntities(ids, fetch2) {
    ...
    const entities = [];
    for (const id of ids) {
      const entity = await withRetry(() => fetch2(id), {}, this.config.logger);
...
async function createStripeWebSocketSession(apiKey, logger) {
  ...
  async function connect() {
    ...
    try {
      ...
      ws.on("close", (code, reason) => {
        ...
        if (!isClosing) {
          const delay = session.reconnectDelay * 1e3;
