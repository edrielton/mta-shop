...
// ─── Auth helper — JWT (Authorization: Bearer) OU sessão (cookie) ────────────
function getUserIdFromRequest(req: Request): string | null {
  const authHeader = req.headers.authorization;
  if (authHeader?.startsWith("Bearer ")) {
    const token = authHeader.slice(7);
    const secret = process.env.JWT_SECRET || process.env.SESSION_SECRET || "mta-jwt-secret-dev";
    try {
    ...
    for (const file of files) {
      ...
      if (existing.length > 0) {
        ...
        const [updated] = await db
          .update(mods)
          .set({ filename: file.filename, size: file.size, uploadedAt: new Date() })
        ...
        const [newMod] = await db
          .insert(mods)
          .values({ originalName: file.originalname, filename: file.filename, size: file.size, category, active: true })
          .returning();
...
function detectCategory(ext: string): string {
  const map: Record<string, string> = {
    ".dff": "modelo", ".txd": "textura", ".col": "colisão",
    ".ifp": "animação", ".lua": "script", ".zip": "pacote",
    ".rar": "pacote", ".img": "imagem", ".ide": "definição", ".ipl": "mapa",
  };
