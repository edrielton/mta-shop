...
export interface IStorage {
  ...
  // Player tokens (auto-login)
  upsertPlayerToken(data: { serial: string; token: string; expiresAt: Date }): Promise<void>;
  getPlayerToken(token: string): Promise<PlayerToken | undefined>;
  markPlayerTokenUsed(token: string): Promise<void>;

  // Player data (RP sync)
  upsertPlayerData(data: Partial<PlayerDataRow> & { serial: string }): Promise<void>;
  getPlayerData(serial: string): Promise<PlayerDataRow | undefined>;
...
export class DatabaseStorage implements IStorage {
  ...
  async upsertPlayerToken(data: { serial: string; token: string; expiresAt: Date }): Promise<void> {
    await db.insert(playerTokens).values({
      serial:    data.serial,
      token:     data.token,
      expiresAt: data.expiresAt,
    }).onConflictDoUpdate({
      target: playerTokens.token,
      set: { serial: data.serial, expiresAt: data.expiresAt, used: false },
    });
  }

  async getPlayerToken(token: string): Promise<PlayerToken | undefined> {
    const [row] = await db
      .select()
      .from(playerTokens)
      .where(eq(playerTokens.token, token));
    return row || undefined;
  }

  async markPlayerTokenUsed(token: string): Promise<void> {
    await db.update(playerTokens).set({ used: true }).where(eq(playerTokens.token, token));
  }

  // ── PLAYER DATA ─────────────────────────────────────────────────

  async upsertPlayerData(data: Partial<PlayerDataRow> & { serial: string }): Promise<void> {
    await db.insert(playerData).values({
      ...data,
      updatedAt: new Date(),
    } as any).onConflictDoUpdate({
      target: playerData.serial,
      set: { ...data, updatedAt: new Date() } as any,
    });
  }

  async getPlayerData(serial: string): Promise<PlayerDataRow | undefined> {
    const [row] = await db.select().from(playerData).where(eq(playerData.serial, serial));
    return row || undefined;
  }

  // ── USER BY MTA SERIAL ──────────────────────────────────────────

  async getUserByMtaSerial(serial: string): Promise<User | undefined> {
    const [user] = await db.select().from(users).where(eq(users.mtaSerial, serial));
    return user || undefined;
  }
